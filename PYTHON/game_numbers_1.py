...
while True :
	...
	if first_player.lower()  == 'PC'.lower() :
		input_value=1
		(x,y,input_value)=player_entry(bool(1),range_input,input_value,intel_list)
		player=bool(1)
	else :
		input_value=input( first_player + ': Enter the input within the  range of '.upper() + str (x) +' to '+ str(y) + ' : ' )
		player=bool(0)
	
	if validate_input(x,y,input_value) :
                        print 'Kindly enter the proper input'.upper()

	while True :
		player= not(player)
                (x,y,input_value)=player_entry(player,range_input,input_value,intel_list)
